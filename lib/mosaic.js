...
// Raw (undecoded-on-reuse) pixel data rather than PNG-encoded, since each
// thumbnail is composited onto the canvas once per matching grid cell —
// often dozens/hundreds of times for a small tile set — and sharp has to
// fully decode an encoded `input` buffer on every single composite() use.
// Handing it already-decoded raw RGB avoids that repeated decode cost.
