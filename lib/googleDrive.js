...
async function downloadFile(fileId, apiKey) {
  ...
  const res = await fetch(url, { signal: AbortSignal.timeout(config.externalFetchTimeoutMs) });
  if (!res.ok) {
...
export async function fetchDriveFolderTiles(folderInput, count) {
  ...
  let res;
  try {
    res = await fetch(listUrl, { signal: AbortSignal.timeout(config.externalFetchTimeoutMs) });
  } catch {
